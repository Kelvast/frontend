 ...
 *          emitPlayerMoveAcked({ path: msg.path, pace: msg.pace });
 *          break;
 *
 ...
 *          return onPlayerMoveAcked(({ path, pace }) => {
 ...
 *        emitInputIntent({ intent: 'orbit-left' });
 *
 *        // systems/camera.ts
 ...
 *   - To add an event: add it here, add to GAME_EVENT, add emitX + onX, emit
 *     in the inbound/system, subscribe in the system.
 */
export interface GameEventMap {
  ...
  "input:intent": {
    intent: "orbit-left" | "orbit-right" | "orbit-up" | "orbit-down" | "zoom-in" | "zoom-out";
  };
  ...
  // ── Bus ───────────────────────────────────────────────────────────────────
  // Emitter: bus.ts (internal)  |  Listener: dev monitor, error reporting
  // Fired when any listener throws. Never fired recursively — if a bus:error
  // listener itself throws, the error is logged and swallowed.
  "bus:error": { event: GameEventKey; err: unknown };
}
